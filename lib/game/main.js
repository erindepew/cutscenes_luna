ig.module(
    ...
    .defines(function(){
        ...
        MyGame = ig.Game.extend({
            ...
            entity_pos: [[0 ,40],[300, 270], [-50 ,170],[200 ,250],[400 ,230]],

            //counter for which segment

            segment_num: 0,

            //variables for calculating mouse movement
            ...
            onBonusObj: function(mousex, mousey){
                console.log( "return of bonus: " + ig.game.getEntityByName('bonus'));
                var bonus = ig.game.getEntityByName('bonus');
                if (bonus !== undefined) {
                    return (mousex > bonus.pos.x
                        && mousex < bonus.pos.x + bonus.size.x
                        && mousey > bonus.pos.y
                        && mousey < bonus.pos.y + bonus.size.y);
                }
            },
            ...
            update: function() {
               ...
               // function for changing scenes
                ...
                //calculate mouse velocity function

                if (ig.input.state('click') && ig.input.mouse.x > 0 && ig.input.mouse.x < ig.system.width){
                        this.prev_posx  = this.current_posx;
                        this.current_posx = ig.input.mouse.x;
                        if (this.prev_posx != 0){
                            this.mouse_velocity =  this.current_posx - this.prev_posx;
                        }
                }
                else {
                    this.mouse_velocity = 0;
                    this.prev_posx = 0;
                    this.current_posx = 0;
                }

                if (ig.input.pressed('click') && ig.game.onBonusObj(ig.input.mouse.x, ig.input.mouse.y)){
                    window.alert("Congrats! You found a bonus!");
                }
            },
